...
echo. >> QZ-Companion-log.txt
adb shell pidof org.cagnulein.qzcompanionpeloton | findstr /r /c:"[0-9]*" >nul
if %errorlevel% == 0 (
  echo QZ Companion found - removing | tee -a QZ-Companion-log.txt
  adb uninstall org.cagnulein.qzcompanionpeloton >> QZ-Companion-log.txt 2>&1
) else (
...
echo. >> QZ-Companion-log.txt
adb install QZCompanionPeloton.apk 2>&1 | tee -a QZ-Companion-log.txt
timeout 5

echo. | tee -a QZ-Companion-log.txt
echo Launching QZ Companion ... | tee -a QZ-Companion-log.txt
echo. >> QZ-Companion-log.txt
adb shell monkey -p org.cagnulein.qzcompanionpeloton 1 >> QZ-Companion-log.txt 2>&1
timeout 5
...
adb shell dumpsys package org.cagnulein.qzcompanionpeloton > dumpsys.log 2>&1
